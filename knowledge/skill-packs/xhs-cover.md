...
- 多页知识卡片、教程组图或可截图 HTML 交给 `xhs-carousel`；本专家只负责首图。

## 多版本测试

测试版只改变一个清楚变量，例如主体位置、标题层级或背景信息密度；其余输入保持
一致。每版都检查主体保真、标题准确、缩略图可读和未授权元素。随机生成多张但同时
改变所有变量，不算可归因的测试。

本节测试纪律受 `spacezephyr-creator-buddy-xhs-skills` 启发，没有复制其提示词或资产。
